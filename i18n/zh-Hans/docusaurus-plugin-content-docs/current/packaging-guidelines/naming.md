...
通常情况下，软件包名称应当尽可能遵循上游项目的正式名称。

软件包名称应该为小写，并且优先使用短横线 (`-`) 而非下划线 (`_`)。具体的大小写规则参见下文的[大小写](#大小写)章节。您可以从上游源码包 (tarball) 的名称、该软件来源的项目名称，以及过去其他发行版/打包者用于此软件包的名称中获取一些线索。
...
打包者不得将上游项目正式名称本身所包含的数字 (例如 `libxml2`) 视为 ABI 或版本号编码。
